...
<main>
<h2>XML,JSON<br>simplexml_load_fileを学習する</h2>
<h3>XMLの情報を読み込む</h3>
<pre>
  <?php
    // XMLオブジェクト読込 = simplexml_load_file('SMLファイル名');
    $xmlTree = simplexml_load_file('https://h2o-space.com/feed');
    // ↑例えば、以上のリンクなら読込方法は以下の通り。

    // ブログ名は「title」、ブログ内容は「item」要素にある。
    // そして、両方とも「channel」要素内にある。
    // といっても、ほぼ「htmlの階層順に書いていく」と覚えればよし。

    $xmlTree->channel->item[0]->title
    // itemに[]があるのは、ブログの記事数だけitemがあるので配列にしているから。
   ?>
  <?php
    // サンプルコードはこんな感じ
    $xmlTree = simplexml_load_file('https://h2o-space.com/feed');
   ...
   <!-- 例えば「$item->link」なら、「$xmlTree->channel->item->link」と同義に。 -->
  <?php endforeach; ?>
  <h3>JSONの情報を読み込む</h3>
  <p>json = JavaScript Object Notation</p>
  <p>XMLより簡単にできるので覚えておきたい</p>
  <?php
    // JSONオブジェクト読込 = file_get_contents('https://h2o-space.com/feed/json');
    $file = file_get_contents('https://h2o-space.com/feed/json');

    // json_decodeを使って、コードから日本語に変換する。
    // ちなみに、聞いたこともある「encode(エンコード)」は
    // 日本語からコードへ、ということに。

    $json = json_decode($file);
   ?>
   <?php
   // foreach文+htmlを使ってブラウザ画面内に表示！
