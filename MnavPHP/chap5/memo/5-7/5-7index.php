...
<body>
  ...
  <h2>Practice</h2>
  <form action="5-7input_do.php" method="post">
  ...
  <pre>
    ...
    // ページに移動できるようにする。
    //http://localhost/MnavPHP/MnaviPHP/chap5/memo/5-7/5-7index.php?page=4
    //
    ...
    <?php
     ...
     ?>
      ...
      <?php //lesson7:ifを使って省略された時に「…」を補う ?>
       ...
       <?php while ($memo = $memos->fetch()): ?>
         <p>
           <a href="5-7memo.php?id=<?php print($memo['id']); ?>">
       ...
       <?php
        ...
        // 任意の―ページへ移動できるようにするためには…？
         ?>
         <!-- <a href="5-7index.php?page=<?php// print($page-1) ?>"><?php// print($page-1) ?>ページ目へ</a>|<a href="5-7index.php?page=<?php// print($page+1) ?>"><?php// print($page+1) ?>ページ目</a> -->
        ...
        // 1.は上コードでいう「前ページ($page-1)へ行くリンク」を細工するんだな。
          ?>
          <?php if ($page >= 2): ?>
            <a href="5-7index.php?page=<?php print($page-1) ?>"><?php print($page-1) ?>ページ目へ</a>
          <?php endif; ?>
            <?php
      ...
      <!-- ↑のコードをさらにこう↓する。 -->
      <p><a href="5-7memo.php?id=<?php print($memo['id']); ?>"><?php print(mb_substr($memo['memo'], 0, 10)); ?></a></p>
      <?php
