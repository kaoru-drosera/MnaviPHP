...
<h3>XXX</h3>
<?php
  // フォームに入力した名前を取得するには、以下の2つのコードを用いる。
  // $_REQUEST['my_name'];
  // $_GET['my_name'];
 ?>
<?php
  print(htmlspecialchars($_GET['my_namae'], ENT_QUOTES) .'<br>');
  // $_REQUEST[]、$_GET[]を使う時は、必ず
  // 「(htmlspecialchers($_REQUEST['my_name']), ENT_QUOTES)」
  // で囲うこと。セキュリティ被害を受けます。
 ?>
<?php
  print(htmlspecialchars($_GET['message'], ENT_QUOTES).'<br>');
  // $_REQUEST[]、$_GET[]を使う時は、必ず
  // 「(htmlspecialchers($_REQUEST['my_name']), ENT_QUOTES)」
  // で囲うこと。セキュリティ被害を受けます。
