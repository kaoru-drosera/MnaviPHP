...
<?php
  ...
  echo "<h4>文章と組み合わせで書く</h4>";
    ...
    // 注意:タイムゾーンの設定が必要なパラメータ
    echo "<h4>タイムゾーンの設定が必要なパラメータ</h4>";
    // 「date_default_timezone_set('（任意）');」はタイムゾーンを設定するパラメータ。
    // 省略してしまうとエラーを吐く場合がある。
    echo "<h4>「date_default_timezone_set('（任意）');」はタイムゾーンを設定するパラメータ。
    <br>省略してしまうとエラーを吐く場合がある。</h4>";
    date_default_timezone_set('Asia/Tokyo');
    print('現在は' . date('G時 i分 s秒') . 'です');
    echo "<br>";
    echo "<p>最初で表示される時刻がおかしいのはタイムゾーンが設定されていないから。</p>";
    echo "<p>タイムゾーンを設定して初めて正しい時刻になる。</p>";
