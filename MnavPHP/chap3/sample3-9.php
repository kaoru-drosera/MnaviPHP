...
<pre>
  <?php
    ...
    print($fruits['lemon'].'<br>');
    // 数字は使えない。インデックスには連想配列内の左側の値を入力する必要がある。
   ?>
  <?php
    $fruits = [
      'apple' => 'りんご',
    ...
    foreach($fruits as $englich => $japanese){
      print($japanese.'　は　えいご　で　'.$englich.'<br>');
    }
    // foreach文。配列内の数字をすべて出したいときには便利だ！
   ?>
  <?php
    $OSs = [
      'win' => 'windows',
    ...
    foreach($OSs as $amt => $official){
      print('<br>');
      print('以下、'.'<br>');
      print($official.'は'.$amt.'<br>');
      print('と表記します。'.'<br>');
    }
    // foreach文。配列内の数字をすべて出したいときには便利だ！
    // {}内のコードは繰り返されるため、
   ...
   <!-- たのしーでしょ？ねぇたのしーでしょ？
   こーんなかきかたもできるよ！ -->
   <?php foreach($OSs as $amt => $official): ?>
     <!-- 後ろの「:（;は間違い）」を忘れない。 -->
    <!-- ここに、繰り返したい処理を書く。 -->
   <?php endforeach ?>
</pre>
