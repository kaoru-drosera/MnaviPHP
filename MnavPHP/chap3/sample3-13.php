...
<main>
<h2>file_put_contentsを学習する</h2>
<h3>ファイルに内容を書き込む</h3>
<pre>
  <?php
    // file_put_contents(ファイルパス名,書き込む内容);
    // これで、自動でファイルが形成されて、内容が書き込まれる。

    // ちなみに、「htdocs」や「httpdoc」「www」等に保存すると
    // 覗きみられる可能性がありセキュリティ面でおすすめしない。
    // フォルダ「htdocs」の外に保存するようにしたい。

    // …といっても今はちょっと完全理解は難しいかも
     ?>
  <?php
      // file_put_contents('C:\xampp\data_by_MnavPHP\news_data\news.text','比較的安全にファイルを保存できます');
      file_put_contents('C:\xampp\data_by_MnavPHP\news_data\news.text','こんな風にね');

     ?>
  <?php
      $success = file_put_contents('C:\xampp\data_by_MnavPHP\news_data\news.text','2018-02-24 ホームページをリニューアルしました');
      if($success){
     ...
     ?>
     <p>うわぉ！ほんとにファイルに書き込めちゃったぞ！</p>
     <p>ただ、ディレクトリまで自動で作ってくれると思ってたからちょっと残念だったな！</p>
     <p>もう一度読みこんでみたら、最初にこれで読み込んだデータは消えちゃったぞ</p>
</pre>
<h3>XXX</h3>
<?php
